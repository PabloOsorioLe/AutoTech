...
namespace BackendCore.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult GeneratePasswordHash([FromBody] PasswordModel model)
        {
            if (string.IsNullOrWhiteSpace(model?.Password))
                return BadRequest(new { Message = "Debe ingresar una contraseña" });

            var user = new User(); // puedes ajustar si requieres algo especial para el user
        ...
        public IActionResult ValidatePassword([FromBody] ValidatePasswordModel model)
        {
            if (string.IsNullOrWhiteSpace(model?.Password) || string.IsNullOrWhiteSpace(model?.PasswordHash))
                return BadRequest(new { Valid = false, Message = "Debe ingresar password y hash." });

            var user = new User(); // puedes ajustar si necesitas algún dato especial
            var result = _passwordHasher.VerifyHashedPassword(user, model.PasswordHash, model.Password);

            if (result == PasswordVerificationResult.Success)
                return Ok(new { Valid = true, Message = "Contraseña válida para el hash." });
            else
                return BadRequest(new { Valid = false, Message = "Contraseña inválida para el hash." });
        }
